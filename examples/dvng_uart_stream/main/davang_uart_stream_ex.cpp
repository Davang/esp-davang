...
{
	dvng::c_uart_stream stream = dvng::c_uart_stream( dvng::uart_stream::s_uart_port<0>() );

	std::string std_string = "this a std string";
	bool _t = true;
	bool _f = false;
	char chararacters[] = "character sequence";
	char chararacter = '@';
	uint8_t u8 = 1;
	uint16_t u16 = 300;
	uint32_t u32 = 70'000;
	...
	stream << "_t : " << _t << dvng::uart_stream::END_LINE::LF;
	stream << "_f : " << _f << dvng::uart_stream::END_LINE::LF;
	...
	stream << dvng::uart_stream::NUM_BASE::HEX << dvng::uart_stream::END_LINE::LF;
	...
	stream << dvng::uart_stream::NUM_BASE::DEC << dvng::uart_stream::END_LINE::LF;
	...
	stream << "f : " << f <<  dvng::uart_stream::END_LINE::LF;
	stream << dvng::uart_stream::s_dec_precision{12} << "d : " << d <<  dvng::uart_stream::END_LINE::LF;

	stream << __PRETTY_FUNCTION__ << " " << __LINE__ <<  dvng::uart_stream::END_LINE::LF;
}